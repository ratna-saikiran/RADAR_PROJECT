%% Ratna Sai Kiran
%% Radar Projecyt
%% M*M MCPC Signal implementation train of pulses
...
N_samp_nu=round(PRI_tot/delta_sec);

%% number of samples where bit period
N_samp_bit=round(tb_sec/delta_sec);

%% time axis 
taxis_sec=0:delta_sec:PRI_tot-delta_sec;

%% transmitted signal
Stx_volts=zeros(1,N_samp_nu);

%% phasers order
N_phase=[3,5,2,1,4;5,2,1,4,3;2,1,4,3,5;1,4,3,5,2;4,3,5,2,1];

%% intital phase
S_phi=0;
for mk=1:5
    for n=1:5
        Stx_volts((mk-1)*PRI_sec+1:mk*PRI_sec)=Stx_volts((mk-1)*PRI_sec+1:mk*PRI_sec)+exp(j*2*pi*Fc_Hz*(3-n)*taxis_sec((mk-1)*PRI_sec+1:mk*PRI_sec));
        for m=1:M
            Stx_volts((m-1)*N_samp_bit+1:m*N_samp_bit)=Stx_volts((m-1)*N_samp_bit+1:m*N_samp_bit).*exp(j*M_phase(N_phase(mk,n),m));
        end
    end
end
...
time_acf_sec=-PRI_tot+delta_sec:delta_sec:PRI_tot-delta_sec;


Sout_volts=xcorr(Stx_volts,Stx_volts);
Sout_volts_db=10*log10(abs(Sout_volts));
